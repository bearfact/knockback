...
test 'calls the #{name}_observable method on the target with the source model', ->
  equal @relation.ref(), @target.author_observable()
...
test 'calls the #{name}_observable method on the target with the source collection', ->
  deepEqual @relation.ref().models, @target.comments_observable()

test 'target#{name}_observable updates when source collection is reset', ->
  expect 2
  ...
  @target.comments_observable.subscribe ->
    ok true, "triggered comments_observable 'subscribe' event"

  commentsCollection.reset newComments
  comments = _.map(@target.comments_observable(), (c) -> c.get('content'))
  deepEqual comments, ['yay', 'nay']

test 'target#{name}_observable updates when an item is added to the source collection', ->
  expect 2
  commentsCollection = @relation.ref()
...
test 'target#{name}_observable updates when an item is removed from the source collection', ->
  expect 2
  commentsCollection = @relation.ref()
